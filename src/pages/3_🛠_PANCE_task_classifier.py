...
@st.cache_resource
def load_model():
    return ktrain.load_predictor('models/pance_task')
...
with st.sidebar:
    uploaded_file = st.file_uploader("Upload .csv, .xlsx files not exceeding 50 MB",
                                     accept_multiple_files=False,key="task_file")
...
def make_predictions(text):
    return pance_task_model.predict(text, return_proba=True)
...
def generate_report(df, model, text_col="clean_text"):
    ...
    st.dataframe(df)
    csv = convert_df(df)
    st.sidebar.download_button("Download CSV", csv, "pance_task_processed.csv", "text/csv",
                               key='download-csv')
...
with st.spinner('loading Model...'):
    pance_task_model = load_model()
    labels = pance_task_model.get_classes()
    explainer = lime_text.LimeTextExplainer(class_names=labels, verbose=True)

st.title("PANCE Blueprint Task Categories classifier")
hide_footer()
...
if submit_button and text_input:
    with st.spinner('calculating...'):
        p = pance_task_model.predict(text_input, return_proba=True)
        pred = labels[np.argmax(p)]
    save_prediction("pance_task_category",text_input,pred)
...
if test_dataset:
    ...
    generate_report(df, pance_task_model)
...
if uploaded_file is not None:
    ...
    if ext:
        ...
        if filesize <= 10:
            ...
            if text_col != "<select>":
                # generate report
                generate_report(df, pance_task_model, text_col)
        else:
            st.error(
