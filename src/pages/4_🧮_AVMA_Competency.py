...
from utils import add_logo, hide_footer, page_config, is_Authenticated
...
@st.cache_resource
def load_model():
    return ktrain.load_predictor('models/grad_outcome')
...
with st.sidebar:
    uploaded_file = st.file_uploader("Upload .csv, .xlsx files not exceeding 50 MB",
                                     accept_multiple_files=False,key="avma_file")
...
def make_predictions(text):
    return ngo_model.predict(text, return_proba=True)
...
def generate_report(df,model,text_col="question"):
    """_summary_
    ...
    with st.spinner('Generating Report...'):
        ...
        df[f'predicted_outcome'] = pred
        pr = ProfileReport(df, minimal=False)

    st.header("Dataframe")
    st.dataframe(df)
    csv = convert_df(df)
    st.sidebar.download_button("Download CSV",csv,"avma_processed.csv","text/csv",
    key='download-csv')
...
with st.spinner('loading Model...'):
    ngo_model = load_model()
    labels = ngo_model.get_classes()
    explainer = lime_text.LimeTextExplainer(class_names=labels, verbose=True)

st.title("New Graduate Outcomes / AVMA Competency Rubrics")
col1, col2 = st.columns(2)
with col1:
    with st.form(key='avma_form'):
        text_input = st.text_area('Add Text')
...
if submit_button and text_input:
    with st.spinner('calculating...'):
        p = ngo_model.predict(text_input, return_proba=True)
        pred = labels[np.argmax(p)]
    save_prediction("avma_competency_rubrics",text_input,pred)
...
if test_dataset:
    df = pd.read_csv("src/avma_test_df.csv")
    generate_report(df, ngo_model)
...
if uploaded_file is not None:
    ...
    if ext:
        ...
        if filesize <= 10:
            ...
            if text_col != "<select>":
                # generate report
                generate_report(df, ngo_model, text_col)
        else:
            st.error(
    ...
    st.info('Upload your data in the left sidebar, select text column to generate profiling')
